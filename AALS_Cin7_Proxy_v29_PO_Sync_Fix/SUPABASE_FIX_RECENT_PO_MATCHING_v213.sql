-- AALS Operations v213 - recent PO matching + Cin7 PO read-only synchronization
-- Run once in Supabase SQL Editor before deploying the v213 portal and v29 proxy.
...
create or replace function public.get_aals_po_updates_v213(
  p_entry_type text,
  p_entry_id text
)
returns table (
...
begin
  ...
  v_entry_created_text := coalesce(
    v_record->>'created_at', v_record->>'created_date', v_record->>'order_date',
    v_record->>'date', v_record->>'updated_at', ''
  );
  if v_entry_created_text ~ '^\d{4}-\d{2}-\d{2}' then
    v_entry_date := substring(v_entry_created_text from 1 for 10)::date;
  elsif v_entry_created_text ~ '^\d{1,2}/\d{1,2}/\d{4}' then
    v_entry_date := to_date(split_part(v_entry_created_text, ' ', 1), 'MM/DD/YYYY');
  end if;
  ...
  with raw_candidates as (
    select p.*,
      case
        when btrim(coalesce(p.date_ordered, '')) ~ '^\d{1,2}/\d{1,2}/\d{4}$' then to_date(p.date_ordered, 'MM/DD/YYYY')
        when btrim(coalesce(p.date_ordered, '')) ~ '^\d{4}-\d{2}-\d{2}' then substring(p.date_ordered from 1 for 10)::date
        when btrim(coalesce(p.request_date, '')) ~ '^\d{1,2}/\d{1,2}/\d{4}$' then to_date(p.request_date, 'MM/DD/YYYY')
        when btrim(coalesce(p.request_date, '')) ~ '^\d{4}-\d{2}-\d{2}' then substring(p.request_date from 1 for 10)::date
        when p.cin7_modified_at is not null then p.cin7_modified_at::date
        else p.updated_at::date
      end as po_effective_date,
      exists (
    ...
    select raw_candidates.*,
      po_effective_date between (v_entry_date - 30) and (v_entry_date + 180) as recent_match
    from raw_candidates
  )
  select
  ...
  where
    ...
    or (p.recent_match and p.reference_match)
    or (p.recent_match and p.work_order_match)
    or (
      v_waiting_materials
      and p.recent_match
  ...
  order by
    case
      ...
      when p.recent_match and p.reference_match then 1
      when p.recent_match and p.work_order_match then 2
      else 3
    end,
    p.updated_at desc;
end;
...
revoke all on function public.get_aals_po_updates_v213(text, text) from public;
grant execute on function public.get_aals_po_updates_v213(text, text) to authenticated;
