...
## v28: Cin7 Purchase Orders (read-only)

The proxy can now read Cin7 Purchase Orders and cache them in the Operations
portal. The integration uses only `GET /api/v1/PurchaseOrders` against Cin7;
it contains no Cin7 PO write operation.

- `POST /api/sync-cin7-purchase-orders-to-operations`
- `GET /api/cin7-purchase-orders-sync-status`

Both routes require an authenticated AALS admin. Run
`SUPABASE_ADD_CIN7_PO_READONLY_SYNC_v212.sql` before using them. Existing
manual PO records remain local to Operations and are never sent to Cin7.
